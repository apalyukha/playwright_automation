...
class WebService:
    ...
    def _get_token(self, url: str):
        ...
        match = re.search(
            '<input type="hidden" name="csrfmiddlewaretoken" value="(.+?)">', rsp.text
        )
        if match:
    ...
    def login(self, login: str, password: str):
        ...
        data = {"username": login, "password": password, "csrfmiddlewaretoken": token}
    ...
    def report_test(self, test_id: int, status: str):
        self.session.post(
            self.base_url + f"/tests/{test_id}/status", json={"status": status}
        )
